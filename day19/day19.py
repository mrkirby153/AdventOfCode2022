...
for line in input_data:
    ...
    blueprint_id, ore_cost, clay_cost, robot_cost_ore, robot_cost_clay, geode_cost_ore, geode_cost_obsidian = map(int, match.groups())
    #dprint(blueprint_id, ore_cost, clay_cost, robot_cost_ore, robot_cost_clay, geode_cost_ore, geode_cost_obsidian)
    blueprints[blueprint_id] = Blueprint(ore_cost, clay_cost, (robot_cost_ore, robot_cost_clay), (geode_cost_ore, geode_cost_obsidian))
...
def get_max_geodes(blueprint, time):
    ore_cost, clay_cost, (obsidian_cost_ore, obsidian_cost_clay),(geode_cost_ore, geode_cost_obsidian) = blueprint.ore_cost, blueprint.clay_cost, blueprint.obsidian_cost, blueprint.geode_cost
    best = 0
    ...
    while queue:
        ...
        # Do nothing
        queue.append((ore + ore_bots, clay + clay_bots, obsidian + obsidian_bots, geodes + geode_bots, ore_bots, clay_bots, obsidian_bots, geode_bots, time - 1))

        if ore >= ore_cost: # Buy an ore bot
            queue.append((ore - ore_cost + ore_bots, clay + clay_bots, obsidian + obsidian_bots, geodes + geode_bots, ore_bots + 1, clay_bots, obsidian_bots, geode_bots, time - 1))

        if ore >= clay_cost: # Buy a clay bot
            queue.append((ore - clay_cost + ore_bots, clay + clay_bots, obsidian + obsidian_bots, geodes + geode_bots, ore_bots, clay_bots + 1,obsidian_bots, geode_bots, time - 1))

        if ore >= obsidian_cost_ore and clay >= obsidian_cost_clay: # Buy an obsidian bot
            queue.append((ore - obsidian_cost_ore + ore_bots, clay - obsidian_cost_clay + clay_bots, obsidian + obsidian_bots, geodes + geode_bots, ore_bots, clay_bots, obsidian_bots + 1, geode_bots, time - 1))

        if ore >= geode_cost_ore and obsidian >= geode_cost_obsidian: # Buy a geode bot
            queue.append((ore - geode_cost_ore + ore_bots, clay + clay_bots, obsidian - geode_cost_obsidian + obsidian_bots, geodes + geode_bots, ore_bots, clay_bots, obsidian_bots, geode_bots + 1, time - 1))
    print()
    return best
